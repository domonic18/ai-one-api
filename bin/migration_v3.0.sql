...
IF @tokens_table_exists > 0 THEN
    ...
    -- 如果索引不存在，则创建
    IF @group_index_exists = 0 THEN
        -- 创建索引以提高查询性能
        CREATE INDEX idx_tokens_group ON tokens(`group`);
        SELECT 'tokens表group字段索引创建成功' as message;
    ELSE
...
ELSE
    SELECT 'tokens表不存在，跳过令牌用户组功能迁移' as message;
END IF;
...
IF @extended_logs_exists = 0 THEN
    -- 创建扩展日志表结构（v3.0版本）
    ...
    SELECT 'extended_logs表创建成功' as message;
ELSE
